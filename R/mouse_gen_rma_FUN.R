mouse_gen_rma_FUN <- function (raw_set, sd_val = 0, snames, ct_off, rds_path)
{
  ...
  split_gene_assignment_ENST_FUN <- function(mrna_assignment) {
    str_idx <- grep("ENSMUST", strsplit(mrna_assignment, "\\//")[[1]])
    str_split <- strsplit(mrna_assignment, "\\//")
    str_split[[1]][str_idx]
    sub_enst <- gsub("^\\/ ", "", str_split[[1]][str_idx])
    paste(gsub(" $", "", sub_enst), collapse = ";")
  }
  cat("Creating negative control index (used for all filtering calculations)",
      fill = T)
  neg_idx <- which(rma_cel@featureData@data$probesettype ==
                     "normgene->intron")
  ...
  cat("Matching unique probesets to the different chips",
      fill = T)
  ...
  unique_prbsets_2sd <- prbsets_2sd[match(unique_idx_2sd,
                                          prbsets_2sd)]
  cat("Matching filtered probesets", fill = T)
  filt_prbset_match_2sd <- sapply(filt_prbsets_2sd, function(x) {
    match(unique_prbsets_2sd, x)
  })
  cat(paste0("Filtering out probesets if present in less than ",
             c(100 - ct_off * 100), "% of chips"), fill = T)
  filt_df_comp_2sd <- as.data.frame(matrix(nrow = length(unique_prbsets_2sd),
                                           ncol = 1 + n_sam))
  ...
  filt_df_comp_2sd$noNA <- unlist(apply(filt_df_comp_2sd,
                                        1, function(x) {
                                          length(which(is.na(x) == TRUE))
                                        }))
  to_use_prbsetid_2sd <- filt_df_comp_2sd$probesetid[which(filt_df_comp_2sd$noNA <=
                                                             round(ct_off * c(length(snames))))]
  idx_2sd <- match(to_use_prbsetid_2sd, rma_cel@featureData@data$probesetid)
  rma_cel_2sd <- rma_cel[idx_2sd]
  cat("Removing all remaining control or unannotated probesets",
      fill = T)
  idx_main_2sd <- which(rma_cel_2sd@featureData@data$probesettype ==
                          "main")
  rma_cel_2sd_main <- rma_cel_2sd[idx_main_2sd]
  cat("Adding HUGO genes to featuredata$geneID", fill = T)
  rma_cel_2sd_main@featureData@data$geneid <- map(rma_cel_2sd_main@featureData@data$geneassignment,
                                                  split_gene_assignment_FUN) %>% sapply(unlist)
  cat("Adding ENST to featuredata$mrnaassignment", fill = T)
  rma_cel_2sd_main@featureData@data$ensgene <- map(rma_cel_2sd_main@featureData@data$mrnaassignment,
                                                   split_gene_assignment_ENST_FUN) %>% sapply(unlist)
  ...
  prb_grp_FUN <- function(goi) {
    test_probesets <- rownames(rma_2sd@featureData@data[(rma_2sd@featureData@data$geneid ==
                                                           goi), ])
    t_length <- length(test_probesets)
    test_xprs <- rma_xprs[which(!is.na(match(rownames(rma_xprs),
                                             test_probesets))), ]
    if (t_length == 1) {
      return(test_xprs)
    }
  ...
  genes_unq_idx <- which(duplicated(rma_2sd@featureData@data$geneid) ==
                           FALSE)
