gsa_bar_plot_FUN <- function (idx, gsa_ls, ts = 8, res_path, coi, wd, ht, pix = 600, file_type="pdf")
{
  ...
  if (length(up_idx) >= 10 & length(dn_idx) >= 10) {
    ...
    cmb_df <- rbind(up_df[1:(10 + (10 - length(dn_idx))),
                          ], dn_df[1:length(dn_idx), ])
    cmb_df$PValue <- -log10(cmb_df$PValue)
  }
  else if (length(up_idx) < 10 & length(dn_idx) >= 10) {
    cmb_df <- rbind(dn_df[1:(10 + (10 - length(up_idx))),
                          ], up_df[1:length(up_idx), ])
    cmb_df$PValue <- -log10(cmb_df$PValue)
  }
  else {
  ...
  gsa_p <- ggplot(cmb_df, aes(x = reorder(pathways, PValue),
                              y = PValue, fill = Direction)) + geom_bar(stat = "identity") +
    theme_bw() + scale_fill_manual(values = brewer.pal(8,
                                                       "Set1")[c(3, 5)]) + guides(fill = guide_legend(reverse = TRUE)) +
    ylab("-log10(PValue)") + xlab("Pathways") + theme(text = element_text(size = ts),
                                                      axis.text.y = element_blank(), panel.grid = element_blank(),
                                                      axis.ticks.y = element_blank()) + ggtitle(paste0("Top ",
                                                                                                       dim(cmb_df)[[1]], " enriched ", pthwy_name, " pathways")) +
    geom_text(aes(label = pathways, y = 1, hjust = 0), position = position_nudge(0,
                                                                                 -0.9)) + geom_text(aes(label = NGenes, y = 1, hjust = 0),
