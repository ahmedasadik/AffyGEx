                     ...
                     wts = NULL, pltfrm = "AFFY", res_path) 
{
  gsa_fun <- function(cont, idcs, Mmat, des, weit, msig_l) {
    gsa <- map2(idcs, rep(list(cont), by = msig_l), function(a, 
                                                             b) {
      camera(y = Mmat, index = a, design = des, contrast = b, 
             weights = weit, trend.var = T)
    })
    names(gsa) <- paste(names(cont)[which(cont == 1)], names(idcs), 
                        sep = "_")
    gsa
  }
  ...
  if (platform == "affy") {
    ...
    if (cntrst == "simple") {
      ...
      gsa_all_res_2sd <- map(gsa_all_mapped_idcs, function(id) {
        camera(y = exprs(gsa_2sd_eset), index = id, design = d_m, 
      ...
      if (length(cont_names) > 1) {
        ...
        stopCluster(cl)
        for (i in seq_along(cont_names)){
          walk2(gsa_cont[[i]], paste(res_path, "gsa_", cont_names[i],"_",exprmnt, 
                                     "_res_", names(gsa_all_mapped_idcs), ".txt", 
                                     sep = ""), write.table, sep = "\t")
        }
        cat("GSA comparison files written to disk", fill = TRUE)
        ...
        gsa_all_res_2sd <- map(gsa_all_mapped_idcs, function(id) {
          camera(y = exprs(gsa_2sd_eset), index = id, 
                 design = d_m, contrast = cnt_mat[, 1], trend.var = T)
        })
