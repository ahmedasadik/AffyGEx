                     ...
                     wts = NULL, pltfrm = "AFFY", res_path)
{
  gsa_fun <- function(cont, idcs, Mmat, des, weit, msig_l) {
    gsa <- map2(idcs, rep(list(cont), by = msig_l), function(a,b) {
      camera(y = Mmat, index = a, design = des, contrast = b,
             weights = weit, trend.var = T)
    })
    names(gsa) <- names(idcs)
    gsa
  }
  platform <- tolower(pltfrm)
  if (platform == "affy") {
    cat(paste("Affy", gsub("pd\\.","",rma_obj@annotation),"platform selected",sep="_"), fill = TRUE)
    ...
    gsa_all_mapped_idcs <- map(msig.data.lists, ids2indices,
                               top_t$hEID)
    ...
    if (cntrst == "simple") {
      ...
      gsa_all_res_2sd <- map(gsa_all_mapped_idcs, function(id) {
        camera(y = exprs(gsa_2sd_eset), index = id,
               design = d_m, contrast = 2, trend.var = T)
      })
      walk2(gsa_all_res_2sd, paste(res_path, "gsa_", exprmnt,
                                   "_res_", names(gsa_all_mapped_idcs), ".txt",
                                   sep = ""), write.table, sep = "\t")
      ...
      if (length(cont_names) > 1) {
        ...
        stopCluster(cl)
        for (i in seq_along(cont_names)) {
          walk2(gsa_cont[[i]], paste(res_path, "gsa_",
                                     cont_names[i], "_", exprmnt, "_res_", names(gsa_all_mapped_idcs),
                                     ".txt", sep = ""), write.table, sep = "\t")
        }
        cat("GSA comparison files written to disk",
            fill = TRUE)
        return(gsa_cont)
      }
      else {
        gsa_all_res_2sd <- map(gsa_all_mapped_idcs,
                               function(id) {
                                 camera(y = exprs(gsa_2sd_eset), index = id,
                                        design = d_m, contrast = cnt_mat[, 1],
                                        trend.var = T)
                               })
        walk2(gsa_all_res_2sd, paste(res_path, "gsa_",
                                     exprmnt, "_res_", names(gsa_all_mapped_idcs),
                                     ".txt", sep = ""), write.table, sep = "\t")
        cat("GSA comparison files written to disk",
            fill = TRUE)
        return(gsa_all_res_2sd)
    ...
    if (cntrst == "simple") {
      ...
      cat("Running Camera", fill = TRUE)
      gsa_all_res_2sd <- map(gsa_all_mapped_idcs, function(id) {
        camera(y = rma_obj$M, index = id, design = d_m,
               contrast = 2, trend.var = T)
      })
      walk2(gsa_all_res_2sd, paste(res_path, "gsa_", exprmnt,
                                   "_res_", names(gsa_all_mapped_idcs), ".txt",
                                   sep = ""), write.table, sep = "\t")
      ...
      if (length(cont_names) > 1) {
        ...
        gsa_cont <- parLapplyLB(cl = cl, X = cnt_list,
                                fun = gsa_fun, idcs = gsa_all_mapped_idcs,
                                Mmat = rma_obj$M, des = d_m, msig_l = msigs,
                                weit = wts)
        stopCluster(cl)
        tt <- map2(gsa_cont, cont_names, function(a,
                                                  b, xprmnt, go_names, res_path = res_path) {
          walk2(a, paste(res_path, "gsa_", b, "_", xprmnt,
                         "_res_", go_names, ".txt", sep = ""), write.table,
                sep = "\t")
        ...
        gsa_all_res_2sd <- map(gsa_all_mapped_idcs,
                               function(id) {
                                 camera(y = rma_obj$M, index = id, design = d_m,
                                        contrast = cnt_mat[, 1], trend.var = T)
                               })
        walk2(gsa_all_res_2sd, paste(res_path, "gsa_",
                                     exprmnt, "_res_", names(gsa_all_mapped_idcs),
                                     ".txt", sep = ""), write.table, sep = "\t")
