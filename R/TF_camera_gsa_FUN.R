                               ...
                               res_path) 
{
  if (pltfrm == "Affy") {
    ...
    TFSBDB_list_enrichment <- camera(y = exprs(gsa_2sd_eset), 
                                     index = TFSBDB_list_idcs, design = d_m, contrast = cntrst, 
                                     weights = wts, trend.var = T)
  }
  else {
    print("Other platform selected")
    TFSBDB_list_idcs <- ids2indices(TF_list, rma_obj$genes$EID)
    TFSBDB_list_enrichment <- camera(y = rma_obj, index = TFSBDB_list_idcs, 
                                     design = d_m, contrast = cntrst, weights = wts, 
                                     trend.var = T)
  }
  if (TF_motifs == TRUE) {
    write.table(TFSBDB_list_enrichment, paste(res_path, 
                                              "TFSBDB_list_enrichment_separate_motifs.txt", sep = ""), 
                sep = "\t")
  }
  else {
    write.table(TFSBDB_list_enrichment, paste(res_path, 
                                              "TFSBDB_list_enrichment_combined_motifs.txt", sep = ""), 
                sep = "\t")
  }
